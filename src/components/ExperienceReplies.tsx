...
const ExperienceReplies = ({ experienceId }: Props) => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", {
        event: "INSERT", schema: "public", table: "experience_replies",
        filter: `experience_id=eq.${experienceId}`,
      }, (payload) => {
        const next = payload.new as Reply;
        setReplies((prev) => (prev.find((p) => p.id === next.id) ? prev : [...prev, next]));
      })
      .on("postgres_changes", {
        event: "DELETE", schema: "public", table: "experience_replies",
        filter: `experience_id=eq.${experienceId}`,
      }, (payload) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast({ title: t("reply.toast.fail"), description: error.message, variant: "destructive" });
      return;
    }
    setName("");
    setComment("");
    toast({ title: t("reply.toast.sent") });
  };
  ...
  return (
    <div className="mt-6 pt-6 border-t border-foam/10">
      ...
      >
        {replies.length > 0
          ? `· ${replies.length} ${t("reply.toggle.count")} ${open ? "▲" : "▼"}`
          : `· ${t("reply.toggle.zero")} ${open ? "▲" : "▼"}`}
      </button>

      {open && (
        <div className="mt-5 space-y-4">
          {replies.map((r) => (
            <div key={r.id} className="border-l-2 border-coral/40 pl-4 py-1">
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-1">
                <span className="font-display text-base text-foam">{r.visitor_name}</span>
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-2">
            <input
              ...
              placeholder={t("reply.name.placeholder")}
              maxLength={80}
              required
            ...
            <textarea
              ...
              placeholder={t("reply.body.placeholder")}
              maxLength={500}
              rows={2}
              required
            ...
            >
              {submitting ? t("reply.submitting") : t("reply.submit")}
            </button>
          </form>
        </div>
      )}
